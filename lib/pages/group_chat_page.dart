...
class GroupChatPageState extends State<GroupChatPage>
  ...
  void _initializeProviders(BuildContext context) {
    ...
    // ── Init Reminders Stream & Background Expired Checks ───────────────
    _initReminderStream();
    _convoReminderStream =
        _reminderProvider.getConversationRemindersStream(groupChatId);
  ...
  void _initReminderStream() {
    final sub =
    _reminderProvider.getActiveReminderCount(_currentUserId).listen((c) {
  ...
  Future<void> _updateGroupBubble(String content, int type,
    ...
    // [SỬA LỖI P0]: Dùng đúng senderId thay vì _currentUserId để lấy tên người gửi
  ...
  void _listenIncomingGroupMessages() {
    ...
    final sub = FirebaseFirestore.instance
      ...
      for (final change in snap.docChanges) {
        ...
        // [SỬA LỖI P1]: Chặn trùng lặp tin nhắn để tránh spam âm thanh
        ...
        // [SỬA LỖI P0]: Truyền đúng tham số senderId vào hàm cập nhật bubble
  ...
  void _startToxicityMonitor() {
    ...
    final sub = FirebaseFirestore.instance
      ...
      // Phân tích reminder (async, non-blocking)
      for (final change in snap.docChanges) {
        if (change.type != DocumentChangeType.added) continue;
        final data = change.doc.data() as Map<String, dynamic>? ?? {};
        final content = data['content'] as String? ?? '';
  ...
  Future<void> _loadSmartReplies() async {
    ...
    try {
      ...
      // Group chat: luôn dùng closeness=3 (không có RelationshipMemory)
      ...
      if (mounted && !resourceManager.isDisposed) {
        final fb = _smartReplyProvider
            .getRuleBasedReplies(messages.first['content']?.toString() ?? '');
        setState(() {
  ...
  Future<void> _triggerZeroTypeSwipe() async {
    ...
    try {
      ...
      if (mounted && !resourceManager.isDisposed) {
        ...
        setState(() {
          _swipeReplies = r.replies; // giữ fallback
        ...
        if (_isShowingSwipeCards)
          Positioned(
              ...
              child: SwipeReplyCards(
                  ...
                  richItems:
                  _swipeRichItems.isNotEmpty ? _swipeRichItems : null,
