...
class AdaptiveChatBubble extends StatefulWidget {
  ...
  // ── Game Center: thêm 2 trường để GameInviteCardBubble/GameResultCardBubble
  //    có thể accept match và navigate đúng.
  ...
  const AdaptiveChatBubble({
    ...
    this.onReact,
    this.onRemoveReact,
    // Game Center fields (default rỗng để tương thích ngược)
...
class _AdaptiveChatBubbleState extends State<AdaptiveChatBubble>
  ...
  void initState() {
    ...
    if (!_isMe &&
        ...
        !widget.message.isGameMessage &&   // ← Game messages không cần AI scan
  ...
  Widget build(BuildContext context) {
    ...
    // ── Game messages: render tanpa animasi swipe & reaksi ────────────────
    // Card game tidak bisa di-reply, di-forward, atau di-react
            ...
            if (_dragOffset > 0)
              Padding(
                ...
                child: Opacity(
                  ...
                  child: Icon(Icons.reply_rounded, color: t.primaryColor, size: 22),
                ),
              ),
          ],
        ),
  ...
  Widget _buildBubbleContent(
    ...
    if (widget.message.type == TypeMessage.voice) {
      return _VoiceBubble(message: widget.message, isMe: _isMe, tokens: t);
    }
    return _MediaBubble(
      message: widget.message,
      isMe: _isMe,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: isMe && !isScam
          ...
          colors: [tokens.myBubbleGradientStart, tokens.myBubbleGradientEnd],
        )
            : null,
        color: isScam
...
class _VoiceBubble extends StatefulWidget {
  ...
  final bool isMe;
  final AppModeTokens tokens;
  const _VoiceBubble({required this.message, required this.isMe, required this.tokens});
  @override
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: widget.isMe
            ? LinearGradient(colors: [widget.tokens.myBubbleGradientStart, widget.tokens.myBubbleGradientEnd])
            : null,
        ...
        children: [
          GestureDetector(
            ...
            child: Container(
              ...
              child: Icon(_playing ? Icons.pause_rounded : Icons.play_arrow_rounded, color: accent, size: 22),
            ),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('0:12', style: TextStyle(color: accent.withOpacity(0.7), fontSize: 11)),
              ],
            ),
          ),
        ],
...
class _WaveformBar extends StatelessWidget {
  final Color color;
  final bool playing;
  static const _heights = [0.4, 0.7, 1.0, 0.6, 0.9, 0.5, 0.8, 0.4, 0.6, 1.0, 0.7, 0.5, 0.9, 0.6, 0.4];
  ...
  Widget build(BuildContext context) => SizedBox(
    height: 20,
    child: Row(
      children: List.generate(_heights.length, (i) => Expanded(
        child: Padding(
          ...
          child: FractionallySizedBox(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: color.withOpacity(playing ? 0.8 + (_heights[i] * 0.2) : 0.45),
...
class _MediaBubble extends StatelessWidget {
  ...
  const _MediaBubble({required this.message, required this.isMe, required this.tokens, this.onTap});
...
class _ImagePreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Hero(
    tag: 'img_$url',
    child: Image.network(url, fit: BoxFit.cover, width: double.infinity, height: 200,
...
class _FileTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 8),
      Flexible(child: Text(content, style: TextStyle(color: textColor, fontSize: fontSize), maxLines: 2, overflow: TextOverflow.ellipsis)),
    ]),
  );
}
...
class _ReplyPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border(left: BorderSide(color: isMe ? Colors.white60 : Colors.blueAccent, width: 3)),
    ),
    child: Text(replyTo.content, maxLines: 2, overflow: TextOverflow.ellipsis,
        style: TextStyle(fontSize: 12, color: isMe ? Colors.white70 : Colors.black54, fontStyle: FontStyle.italic)),
  );
}
...
class _StudyIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 4),
      Text('Study Note', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600,
          color: isMe ? Colors.white70 : Colors.black45, letterSpacing: 0.3)),
    ]),
  );
}
    ...
    child: AnimatedContainer(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 6),
          const Expanded(child: Text('Tin nhắn có dấu hiệu đáng ngờ',
              style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w600, color: Color(0xFF7B3F00)))),
          Icon(isExpanded ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, size: 15, color: const Color(0xFF7B3F00)),
        ]),
        if (isExpanded) ...[
          const SizedBox(height: 6),
          const Text('Hệ thống AI phát hiện tin nhắn này có thể là lừa đảo. Không chia sẻ thông tin cá nhân.',
...
class _DoubleCheck extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Stack(alignment: Alignment.centerRight, children: [
      Positioned(left: 0, child: Icon(Icons.check_rounded, size: size, color: color)),
...
class _PeerAvatar extends StatelessWidget {
  ...
  final double radius;
  final bool isOnline;
  const _PeerAvatar({this.url, required this.primaryColor, required this.radius, this.isOnline = false});
  @override
  Widget build(BuildContext context) => Stack(children: [
    CircleAvatar(radius: radius, backgroundColor: primaryColor.withOpacity(0.15),
        backgroundImage: (url != null && url!.isNotEmpty) ? NetworkImage(url!) : null,
        child: (url == null || url!.isEmpty) ? Icon(Icons.person_rounded, size: radius, color: primaryColor.withOpacity(0.7)) : null),
...
class _ReactionRow extends StatelessWidget {
  ...
  final double fontSize;
  const _ReactionRow({required this.reactions, required this.isMe, required this.primaryColor, this.onTap, required this.fontSize});
  @override
  Widget build(BuildContext context) => Padding(
    padding: EdgeInsets.only(top: 3, left: isMe ? 0 : 44, right: isMe ? 18 : 0),
    child: Wrap(spacing: 4, runSpacing: 3,
        children: reactions.map((r) => GestureDetector(
            ...
            child: Container(
                ...
                child: Text('${r.emoji} ${r.count}', style: TextStyle(fontSize: fontSize + 1, height: 1.2))))).toList()),
  );
}
...
class _ContextMenu extends StatelessWidget {
  ...
  final AppModeTokens tokens;
  final AppMode appMode;
  final VoidCallback onCopy, onReply, onReact;
  final VoidCallback? onForward, onDelete;
  const _ContextMenu({required this.isMe, required this.tokens, required this.appMode,
    required this.onCopy, required this.onReply, required this.onReact, this.onForward, this.onDelete});
  @override
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Center(child: Container(margin: const EdgeInsets.only(top: 10, bottom: 4), width: 36, height: 4,
            decoration: BoxDecoration(color: Colors.black12, borderRadius: BorderRadius.circular(2)))),
        _Item(icon: Icons.copy_rounded, label: 'Sao chép', iconSize: iconSize, fontSize: fontSize, color: tokens.primaryColor,
            onTap: () { Navigator.pop(context); onCopy(); }),
        const _Div(),
        _Item(icon: Icons.reply_rounded, label: 'Trả lời', iconSize: iconSize, fontSize: fontSize, color: tokens.primaryColor,
            onTap: () { Navigator.pop(context); onReply(); }),
        const _Div(),
        _Item(icon: Icons.add_reaction_outlined, label: 'Biểu cảm', iconSize: iconSize, fontSize: fontSize, color: tokens.primaryColor,
            onTap: () { Navigator.pop(context); onReact(); }),
        if (onForward != null) ...[const _Div(), _Item(icon: Icons.forward_rounded, label: 'Chuyển tiếp', iconSize: iconSize, fontSize: fontSize, color: tokens.primaryColor, onTap: () { Navigator.pop(context); onForward!(); })],
        if (onDelete != null) ...[const _Div(), _Item(icon: Icons.delete_outline_rounded, label: 'Xóa tin nhắn', iconSize: iconSize, fontSize: fontSize, color: Colors.red.shade400, onTap: () { Navigator.pop(context); onDelete!(); })],
...
class _Item extends StatelessWidget {
  final IconData icon; final String label; final double iconSize, fontSize; final Color color; final VoidCallback onTap;
  const _Item({required this.icon, required this.label, required this.iconSize, required this.fontSize, required this.color, required this.onTap});
  @override
  Widget build(BuildContext context) => InkWell(
      onTap: onTap, borderRadius: BorderRadius.circular(10),
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
          child: Row(children: [Icon(icon, size: iconSize, color: color), const SizedBox(width: 14),
...
class _Div extends StatelessWidget {
  const _Div();
  @override
  Widget build(BuildContext context) => const Divider(height: 0, thickness: 0.5, indent: 20, endIndent: 20, color: Colors.black12);
}